...
(define (helper w1 w2 acc)
  (if (or (null? w1) (null? w2) (not (eq? (car w1) (car w2))))
      (cons (reverse acc) (list w1 w2))
      (helper (cdr w1) (cdr w2)( cons (car w1) acc))))
...
(define (longest-common-prefix-of-list words)
  (helper-1 words (car words)))

(define (helper-1 words prefix)
  (if (null? (cdr words))
      prefix
      (helper-1 (cdr words) (helper-2 prefix (car (cdr words))))))

(define (helper-2 w1 w2)
  (if (or (null? w1) (null? w2) (not (char=? (car w1) (car w2))))
      '()
      (cons (car w1) (helper-2 (cdr w1) (cdr w2)))))
  ...
  (let ((branch (get-ch-branch st (car pattern))))
    (if branch
        (let* ((label (get-branch-label branch))
               ...
               (prefix (longest-common-prefix label pattern)))
          (let ((result (first prefix))
                (rest-w1 (second prefix))
                (rest-w2 (third prefix)))
            (cond
              ((and (not (null? result)) (null? rest-w1) (null? rest-w2)) #t)
              ((and (null? rest-w1) (not (null? rest-w2))) (list label rest-w2 subtree))
              ((and (not (null? result)) (null? rest-w2)) #t)
              (else (list #f result)))))
        (list #f '()))))
...
(define (st-has-pattern? st pattern)
  (let ((result (match-pattern-with-label st pattern)))
    (cond
      ...
      ((eq? (car result) #f) #f)
      (else
       (let ((new-pattern (car (cdr result)))
             (subtree (car (cdr (cdr result)))))
         (st-has-pattern? subtree new-pattern))))))