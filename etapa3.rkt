...
;; Funcție ajutătoare care generează toate prefixele posibile pentru un sufix dat
(define (longest-common-substring text1 text2)
  (let ((st (text->cst text1)))
    (let ((result (foldl (lambda (suffix longest)
                           (update-longest-for-suffix st suffix longest))
                         (cons '() 0)
                         (get-suffixes text2))))
      (car result))))

(define (prefixes suffix)
  (letrec ((generate-prefixes (lambda (suffix n)
                                (if (= n 0)
                                    '()
                                    (cons (take suffix n)
                                          (generate-prefixes suffix (- n 1)))))))
    (generate-prefixes suffix (length suffix))))



(define (update-longest-for-suffix st suffix longest-so-far)
  (let loop ((prefixes (prefixes suffix)) ; Inițializăm lista de prefixe pentru sufix
             (longest longest-so-far)) ; Cel mai lung subșir comun găsit până acum
    (if (null? prefixes) ; Dacă lista de prefixe este goală, terminăm recursivitatea
        longest ; Întoarcem cel mai lung subșir comun găsit
        (let* ((current-prefix (car prefixes)) ; Prefixul curent din listă
               (new-longest (if (and (st-has-pattern? st current-prefix) ; Verificăm dacă prefixul curent este un pattern în ST
                                     (> (length current-prefix) (cdr longest))) ; și este mai lung decât cel mai lung găsit până acum
                              (cons current-prefix (length current-prefix)) ; Actualizăm cel mai lung subșir comun
                              longest))) ; Altfel, păstrăm valoarea anterioară
          (loop (cdr prefixes) new-longest))))) ; Continuăm recursivitatea cu restul listei de prefixe
