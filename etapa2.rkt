...
(define (get-ch-words words ch)
  (foldr (lambda (word acc)
                (if (and (not (null? word)) (equal? (car word) ch))
                    (cons word acc)
                    acc))
              '()
              words))
...
(define (cst-func suffixes)
  (let* ((prefix (longest-common-prefix-of-list suffixes))
    ...
    (cons prefix
          (map (lambda (suffix) (list-tail suffix size_prefix))
               suffixes))))
...
(define (suffixes->st labeling-func suffixes alphabet)
  (letrec ((helper
            (lambda (char)
              (let ((char_start (filter (lambda (sufix) (and (not (null? sufix)) (equal? (car sufix) char)))
                                                  suffixes)))
                (if (null? char_start)
                    '()
                    (let ((label_s (labeling-func char_start)))
                      (let ((label (car label_s))
                            (new-suffixes (cdr label_s)))
                        (if (null? new-suffixes)
                            (list label)
...
(define (text->st labeling-func)
  (lambda (text)
    (let* ((end (append text '(#\$)))
           (suffixes (get-suffixes end))
           (alphabet (sort (remove-duplicates end) char<?)))
